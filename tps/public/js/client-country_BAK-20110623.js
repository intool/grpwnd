...
function init_selects() {
	$.getJSON('/json_cache/country_list.json', function(data) {
		...
		var c = window.location.pathname.split("/")[1].toUpperCase();
		if (c) {
	...
	var country_id = window.location.pathname.split("/")[1].toUpperCase();
	if (country_id) {
		...
		var get = $.ajax({
			...
			$('#city-select').fadeIn(300, 'swing');	
			  ...
			  error: function (request, status, error) {
				...
				alert('Good Morning Early Bird! \n You are the first person viewing this today! \n I\'m still busy updating the list... won\'t be a minute.');
				buildcitylist(country_id);
			}
		}); // end ajax
	} // end if(country_id)
} // end function init
...
$('#country-select-box').change(function() {
	...
	location.href = '/' +country + '/'; // send to city
...
function buildcitylist(country_id) {
$.get('/getcitylist/' + country_id, function(data) {
  		console.log('Cache refreshed for country :: ' +country_id );
 		// dealinit(city);
		setTimeout( function() {
